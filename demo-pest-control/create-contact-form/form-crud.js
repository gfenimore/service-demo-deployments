 ...
 * - setRecordId(id): a parent-declared create form STAGES the record the bar hands it (s40)
 */
...
(function() {
  ...
  constructor() {
    ...
    // THE STAGED PARENT (s40). Set only through setRecordId on a parent-declared form; the
    // label is read after staging so the form can SAY whose child it adds, never guessed.
    this.parentId = null;
  ...
  async save() {
    ...
    // THE PARENT IS REQUIRED (s40): a parent-declared create form REFUSES to save
    // unparented -- an unscoped child row is the s31 ban restated at the form's grain.
    // Parented, the staged id is written into the declared column with the row.
    const wasCreate = this.mode === 'create';
    if (this.config.parent && wasCreate) {
      if (!this.parentId) {
        this.emit('blueprint:error', {
      ...
      formData[this.config.parent.field] = this.parentId;
    }
    ...
    try {
      ...
      // s40: a parented form's job is ONE child per summons. Reset to a fresh create (createRecord
      // flipped this form to edit mode on the row it just wrote) and return the page to the
      // parent's record -- the shell re-shows the route and this form STANDS DOWN.
      if (this.config.parent && wasCreate) {
        this.currentRecord = null;
        this.recordId = null;
        this.mode = 'create';
        this.render();
        this.emit('blueprint:navigate', { id: this.parentId });
      }
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // THE PARENT (s40): the bar's summons stages the focused record
  // ═══════════════════════════════════════════════════════════════════════

  /**
   * The record bridge, the router and the command bar's summons all land here. Only a
   * parent-declared form takes it: the id is STAGED, said in the form's banner, written into
   * config.parent.field at save, and required before any create (the s31 unscoped ban at the
   * form's grain). On a form declaring no parent this is a deliberate no-op -- the shell
   * hands record ids to every component that can take one, and this one cannot.
   */
  setRecordId(recordId) {
    if (!this.config.parent) return;
    const id = recordId ? String(recordId) : null;
    if (!id || id === this.parentId) return;
    this.parentId = id;
    this.parentLabel = null;
    if (this.container) this.render();
    this._loadParentLabel(id);
  }

  /**
   * Say WHOSE child this form adds. The parent's table and label column follow the naming
   * convention the declared field already carries: <entity>_id -> table <entity>, label
   * <entity>_name. A failed read degrades to showing the id -- it never blocks the form.
   */
  async _loadParentLabel(id) {
    const p = this.config.parent;
    if (!p || !this.supabaseClient) return;
    const entity = p.field.replace(/_id$/, '');
    try {
      const { data, error } = await this.supabaseClient
        ...
        .eq(p.field, id)
        .single();
      if (error || !data || id !== this.parentId) return;
      this.parentLabel = data[entity + '_name'] || data.name || null;
      const banner = this.container && this.container.querySelector('[data-role="parent"]');
      if (banner && this.parentLabel) {
        banner.textContent = 'For ' + entity.replace(/_/g, ' ') + ': ' + this.parentLabel;
      }
    } catch (ignored) { /* the banner keeps the id */ }
  }
  ...
  buildFormHTML() {
    ...
    // s40: a parent-declared create form SAYS whose child it adds -- the staged parent made
    // visible, or the honest wait for the bar's summons. Not decoration: it is the one line
    // that distinguishes "adding a contact to Fredegar Bolger" from an unscoped write.
    let parentBanner = '';
    if (this.config.parent && this.mode === 'create') {
      const parentEntity = this.config.parent.field.replace(/_id$/, '').replace(/_/g, ' ');
      parentBanner = `<div class="blueprint-form-parent" data-role="parent">${
        this.parentId
          ? 'For ' + this.escapeHtml(parentEntity) + ': ' + this.escapeHtml(this.parentLabel || this.parentId)
          : 'Waiting for a ' + this.escapeHtml(parentEntity) + ' -- open this form from its record'
      }</div>`;
    }
  ...
  handleCancel() {
    // s40: a parented create form cancels BACK TO ITS PARENT's record -- the shell re-shows
    // the route and this form stands down. The staged parent survives for the next summons.
    if (this.config.parent && this.mode === 'create' && this.parentId) {
      this.emit('blueprint:navigate', { id: this.parentId });
      return;
    }
    if (this._isDirty) {
