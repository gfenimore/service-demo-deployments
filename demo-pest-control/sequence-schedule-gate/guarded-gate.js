/**
 * Sequence the day - Guarded Gate (contract compliant)
 * Blueprint ID: cf000000-0000-0000-0000-000000000001
 * Pattern: GUARDED-GATE
 * Gate: SCHEDULE.SEQUENCE.CONFIRM
 *
 * Contract:
...
(function () {
  ...
  var GATE = {
  "code": "SCHEDULE.SEQUENCE.CONFIRM",
  "title": "Sequence the day",
  "subject": "The day's stops are put in order from the technician's start, each inside the customer's hour window, with an eta. Accepting writes the order onto the stops; the promises are not sent until you release.",
  "subjectName": {
    "source": "route",
    "key": "route_id",
    "field": "route_name"
  ...
  "verdicts": [
    {
      "code": "SEQUENCE",
      "label": "Sequence and accept",
      "confirm": "Sequence this day and accept the order? Each stop takes its eta.",
      "endpoint": "rpc:sequence_schedule",
      "method": "POST",
      "payload": {
        "action": "sequence"
      },
      "consequence": "A run of kind sequence with the projected lines; the order and the eta written onto the stops; a window that cannot be kept is counted on the run.",
      "reaches": null,
      "tone": "primary",
      "next": {
  ...
  class RouteGateUI {
    constructor() {
      ...
      this.metadata = {
        blueprint_id: 'cf000000-0000-0000-0000-000000000001',
    ...
    mount(container, options) {
      ...
      this.container.classList.add('blueprint-route-gate');
      ...
      console.log('[RouteGateUI] Mounted successfully');
    }
    ...
    unmount() {
      ...
      this.container.classList.remove('blueprint-route-gate');
    ...
    onContext(context) {
      ...
      console.log('[RouteGateUI] Context received:', { role: context && context.role });
    }
    ...
    render() {
      ...
      if (!this.subjectId()) {
        this.container.innerHTML =
          '<div class="blueprint-gate-head">' + esc(GATE.title) +
          '<span class="blueprint-gate-code">' + esc(GATE.code) + '</span></div>' +
          '<div class="blueprint-gate-closed">No route selected. Open one from the list to use this gate.</div>';
        return;
      }
      ...
      html += '<div class="blueprint-gate-subject" data-subject>For ' +
              (name ? esc(name) : 'route: ' + esc(this.subjectId())) + '</div>';
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = RouteGateUI;
  }
  if (typeof window !== 'undefined') {
    window.RouteGateUI = RouteGateUI;
  }
})();
  ...
  if (typeof window !== 'undefined') {
    ...
    window.PatternRegistry['cf000000-0000-0000-0000-000000000001'] = window.RouteGateUI;
  }
