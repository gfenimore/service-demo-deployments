...
(function() {
  ...
  constructor() {
    ...
    this.metadata = {
      ...
      icon: 'list',
      route: '/work_order',
      permissions_required: ['work_order:read'],
      sopf_node: 'unknown',
      role_context_id: '',
      description: 'View and search work_order records'
    };
  ...
  "placeholder": "Search pending work orders..."
},
  ...
  mount(container) {
    ...
    this.container.classList.add('blueprint-work_order-list');
    ...
    console.log('[WorkOrderBlueprintUI] Mounted successfully');
  }
  ...
  unmount() {
    if (!this.container) {
      console.warn('[WorkOrderBlueprintUI] Component not mounted, nothing to unmount');
      return;
    }
    ...
    this.container.classList.remove('blueprint-work_order-list');
    ...
    console.log('[WorkOrderBlueprintUI] Unmounted and cleaned up');
  }
  ...
  onContext(context) {
    ...
    if (previousContext && (
      ...
      console.log('[WorkOrderBlueprintUI] Context changed, reloading data');
      this.loadData();
      ...
      console.log('[WorkOrderBlueprintUI] Persona filters changed, reloading:', context.facet || '(none)');
      this.currentPage = 1;
      this.loadData();
    } else if (previousContext && previousContext.facet !== context.facet) {
      ...
      console.log('[WorkOrderBlueprintUI] Persona changed, re-rendering:', context.facet || '(none)');
      this.render();
    ...
    console.log('[WorkOrderBlueprintUI] Context received:', {
  ...
  emit(eventName, detail) {
    if (!this.container) {
      console.warn(`[WorkOrderBlueprintUI] Cannot emit ${eventName}: not mounted`);
      return;
    }
  ...
  initSupabase(supabaseConfig) {
    if (typeof window.supabase === 'undefined' || !window.supabase.createClient) {
      console.warn('[WorkOrderBlueprintUI] Supabase client library not loaded');
      return;
    }
    ...
    if (window.ShellData && (this.config.database.schema || 'public') === shellSchema) {
      ...
      console.log('[WorkOrderBlueprintUI] using the shell data client');
      return;
    }
    ...
    try {
      ...
      console.log('[WorkOrderBlueprintUI] Supabase client initialized');
    } catch (error) {
      console.error('[WorkOrderBlueprintUI] Failed to initialize Supabase:', error);
    }
  }
  ...
  async loadData(quiet) {
    ...
    if (!this.context) {
      console.warn('[WorkOrderBlueprintUI] Cannot load data: no context');
      return;
    }
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        console.log('[WorkOrderBlueprintUI] Fetch aborted');
        return;
      }
      ...
      this.emit('blueprint:error', {
        ...
        message: `Failed to load work_order: ${error.message}`,
        details: { endpoint: '/work_order_due_list' }
      });
    }
  }
  ...
  async fetchFromSupabase() {
    ...
    for (const [column, allowed] of Object.entries(filters)) {
      if (!this.hasColumn(column)) {
        console.warn(`[WorkOrderBlueprintUI] persona filter on '${column}' skipped -- not a column of work_order_due_list`);
        continue;
      }
  ...
  async fetchFromAPI() {
    ...
    const response = await fetch(`${apiBaseUrl}/work_order_due_list?limit=${pageSize}&offset=${offset}${parentScope}`, {
      headers,
  ...
  generateMockData() {
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      record.service = `Service ${i}`;
      
      record.service_window = `Service Window ${i}`;
      
      record.buffer_days = `Buffer Days ${i}`;
      
      record.state = `State ${i}`;
      
      record.compliance = `Compliance ${i}`;
      
      record.assigned_to = `Assigned To ${i}`;
      
      record.pool_reason = `Pool Reason ${i}`;
  ...
  buildErrorHTML() {
    return `
      <div class="blueprint-message blueprint-error">
        <p>Could not load Work Order: ${this.escapeHtml(String(this.loadError))}</p>
  ...
  visibleFields() {
    ...
    if (visible.length === 0) {
      console.warn(`[WorkOrderBlueprintUI] persona '${code}' permits none of this screen's fields`);
    }
    return visible;
  }
    ...
    return `
      <div class="blueprint-list-view">
        <div class="blueprint-table-container">
          <table class="blueprint-data-table" aria-label="Work Order data table">
            <thead>
              <tr>${headerCells}</tr>
            </thead>
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = WorkOrderBlueprintUI;
  }
  
  // Export to window for browser (replaces any existing instance)
  if (typeof window !== 'undefined') {
    window.WorkOrderBlueprintUI = WorkOrderBlueprintUI;
  }
})();
