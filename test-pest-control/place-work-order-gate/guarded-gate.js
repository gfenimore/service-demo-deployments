...
(function () {
  ...
  var GATE = {
  "code": "WORK_ORDER.PLACE.CONFIRM",
  "title": "Place work order",
  "subject": "Placing puts this work order on the cheapest eligible day of the date you give. The dates list on the work order shows what each date costs and with whom. A date with no room, or outside the window, is refused with the reason.",
  "subjectName": {
    "source": "work_order",
    "key": "work_order_id",
    "field": "summary"
  ...
  "verdicts": [
    {
      "code": "PLACE",
      "label": "Place on this date",
      "confirm": "Place this work order on the date given? It floats on that day until the day is released.",
      "endpoint": "rpc:place_on_date",
      "method": "POST",
      "payload": {
        "action": "place"
      },
      "consequence": "Moves the work order to placed through the scheduler as a hand placement (kind override) with its delta and the cost of leaving; the day's slack falls by one.",
      "reaches": null,
      "tone": "primary",
      "next": {
        "route": "/operations/pending/:id",
        "title": "The work order, placed"
      },
      "argMap": {
        "p_by": "user_id",
        "p_date": "day",
      ...
      "inputs": [
        {
          ...
          "required": true,
          "options": [],
          "help": "An admissible date from the work order's dates; the cheapest technician with room on it takes the work order."
        }
      ]
    }
  ],
