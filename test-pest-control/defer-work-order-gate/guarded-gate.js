...
(function () {
  ...
  var GATE = {
  "code": "WORK_ORDER.DEFER.CONFIRM",
  "title": "Defer work order",
  "subject": "Deferring postpones this occurrence until the date you give, inside its window. The planner resumes it on that date; the window itself does not move.",
  "subjectName": {
    "source": "work_order",
    "key": "work_order_id",
    "field": "summary"
  ...
  "verdicts": [
    {
      "code": "DEFER",
      "label": "Defer work order",
      "confirm": "Defer this work order until the date given? The planner resumes it then.",
      "endpoint": "rpc:defer_work_order",
      "method": "POST",
      "payload": {
        "action": "defer"
      },
      "consequence": "Moves the work order to deferred through the planner, records the reason and the date; the admissible dates begin at the date.",
      "reaches": null,
      "tone": "primary",
      "next": null,
      "argMap": {
        "p_note": "note",
        "p_until": "deferred_until",
      ...
      "inputs": [
        {
          ...
          "required": true,
          "options": [],
          "help": "The date the work may resume (YYYY-MM-DD), inside the window."
        },
        {
          "field": "reason_code",
          "label": "Reason",
          ...
          "help": "From the catalog's defer class."
        },
        {
          "field": "note",
          "label": "Note",
